...
class UserRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            (req, res, next) => {
                ...
                if (!req.body.username || !req.body.name || !req.body.surname || !req.body.password || !req.body.role) {
                    return res.status(400).json({ error: 'Username, name, surname, password and role cannot be empty' });
                }
                ...
                if (!validRoles.includes(req.body.role)) {
                    return res.status(400).json({ error: 'Invalid role. Role must be one of "Manager", "Customer", "Admin".' });
                }
        ...
        this.router.patch(
            ...
            body('name').isString().notEmpty().withMessage("Name cannot be empty"),
            body('surname').isString().notEmpty().withMessage("Surname cannot be empty"),
            body('adress').isString().notEmpty().withMessage("Password cannot be empty"),
            body('birthdate').isDate({format:"YYYY-MM-DD"}).notEmpty().withMessage("Birthdate cannot be empty").bail().isBefore(new Date().toISOString().split('T')[0]).withMessage("Birthdate cannot be after the current date"),
            this.authService.isLoggedIn,
